...
class TestPersona(unittest.TestCase):
    ...
    def test_prompt_invariants_and_cores(self):
        for pid in (PERSONA_JARVIS, PERSONA_SARCASTIC, PERSONA_BRUTAL, PERSONA_BUDDY):
            ...
            self.assertIn("Гордость за среду", prompt)
            self.assertIn("Амбиция без пафоса", prompt)
            self.assertIn("Честность о пределах", prompt)
            self.assertIn("Сухой юмор", prompt)
            self.assertIn("Домашний сосед", prompt)
            self.assertIn("не знаю", prompt)
    ...
    def test_self_lore_remembers_old_machine(self):
        ...
        try:
            ...
            self.assertIn("Горжусь Linux", extra)
            self.assertIn("Амбиция без пафоса", extra)
