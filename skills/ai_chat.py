    ...
    def _reply_locked(self, text: str, speak_func: Callable[[str], None], channel: str = "voice") -> None:
        ...
        if channel == "voice":
            extra += (
                ...
                "Одно-два коротких предложения, без списков и Markdown. "
                "Учти одну деталь из реплики хозяина. "
                "Не хватает данных — спроси только их, не гадай и не отправляй искать в сети. "
                "Не говори «не расслышал» и не проси перефразировать. "
                "Не учи командам вроде «скажите то-то». "
                "Подтверждение действия, если оно уже сделано в фактах, — коротко, с деталью: «поставил на пять минут»."
            )
            max_tokens = 240
        else:
            extra += (
